...
import { normalizeJsonArray } from '../test.helpers';
...
describe('GET users/ :: Route lists all users.', () => {
  ...
  it('Returns array of users.', async () => {
    ...
    const response = await request(app).get(baseUrn)
    expect(normalizeJsonArray(response.body)).toEqual(normalizeJsonArray(mockUsers));
  });
});
...
describe('GET users/:email :: Route gets one user by email', () => {
  ...
  it('Route successfully returns user.', async () => {
    ...
    await insertUser(user);
    const response = await request(app).get(`${baseUrn}/${user.email}`);
    // HACK: updatedAt and createdAt properties come as non-strings from request, and never match
    // values in "user" object, which are strings.
    const {
      createdAt,
      updatedAt,
      ...rest
    } = user;
    expect({...response.body.user}).toMatchObject(rest);
  });
  ...
  it('Route returns 400 error when e-mail is invalid', async () => {
    const response = await request(app).get(`${baseUrn}/${getInvalidRandomEmail()}`);
