...
const RestControllerNameResolver = {
  httpMethods: ['post', 'get', 'put', 'delete'],
  _getGlobPattern: () => `**/src/adapters/REST-controllers/**/@(${RestControllerNameResolver.httpMethods.join('|')}).ts`,
  _resolveControllerName: (fileName: string, filePath: string) => {
    const dirs = filePath.split("/");
    const entity = dirs[dirs.length-2];
    const method = fileName.split('.')[0];
    return `${entity}-${method}-RESTcontroller`;
  }
}
container.loadModules([RestControllerNameResolver._getGlobPattern()], {
  formatName: (name, descriptor) => RestControllerNameResolver._resolveControllerName(name, descriptor.path)
});
